...
# Protobuf/Crypto imports with fallbacks
try:
...
def main():
    ...
    print(f"📝 Generate endpoint: http://localhost:{port}/g?name=Prefix&region=IND&count=3&activate=true")
    print(f"📝 Supported regions: {list(ACTIVATION_CONFIGS.keys())}")
    print("=" * 60)
    print("💡 Quick Examples:")
    print(f"  • Generate 3 accounts for India: http://localhost:{port}/g?region=IND&count=3")
    print(f"  • Generate 5 accounts with auto activation: http://localhost:{port}/g?region=ID&count=5&activate=true")
    print(f"  • Generate 2 accounts without activation: http://localhost:{port}/g?region=TW&count=2&activate=false")
    print("=" * 60)
