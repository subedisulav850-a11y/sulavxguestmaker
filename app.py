...
# =============================================================================
# PROTOBUF & CRYPTO IMPORTS WITH GRACEFUL FALLBACKS
# =============================================================================
...
try:
    ...
    def pad(data, block_size): 
        """Fallback pad function"""
        return data + b'\x00' * (block_size - len(data) % block_size)

# Protobuf imports with comprehensive error handling
...
try:
    ...
    print("⚠️ Running in compatibility mode - some features disabled")
except TypeError as e:
    print(f"⚠️ Protobuf type error: {e}")
    print("⚠️ Python version may be incompatible - using fallback mode")
except Exception as e:
    print(f"⚠️ Unexpected protobuf error: {e}")
    print("⚠️ Using fallback mode")

# Create dummy classes if protobuf is not available
if not NEW_PROTO_AVAILABLE:
    class DummyProto:
        """Dummy protobuf class for fallback mode"""
    ...
    class DummyMajorLogin(DummyProto):
        """Dummy MajorLogin class"""
        pass
        
    class DummyMajorLoginRes(DummyProto):
        """Dummy MajorLoginRes class"""
        pass
...
def create_encryption_cipher():
    """Creates AES cipher for encryption"""
...
async def EnC_Vr(N: int) -> bytes:
    """Encodes a variable-length integer for protobuf"""
    if N < 0: 
        return b''
    H = []
    while True:
...
async def CrEaTe_ProTo(fields: Dict) -> bytes:
    """Creates a protobuf message from field dictionary"""
    p = bytearray()
...
def E_AEs(Pc: str) -> bytes:
    """Encrypts hex string using AES-CBC"""
    if not AES_AVAILABLE:
...
def encrypt_with_aes(data: bytes) -> Optional[bytes]:
...
def build_safe_major_login_payload(open_id: str, access_token: str, region: str, client_ip: str) -> Optional[bytes]:
    """Builds encrypted MajorLogin payload for activation"""
    if not NEW_PROTO_AVAILABLE:
        return None
    try:
...
def major_login_safe(access_token: str, open_id: str, region: str, client_ip: str, proxy_url: Optional[str] = None) -> Optional[bytes]:
    """Performs MajorLogin for activation"""
...
def get_login_data_safe(jwt_token: str, access_token: str, base_url: str, region: str, client_ip: str, proxy_url: Optional[str] = None) -> bool:
    """Performs GetLoginData for final activation"""
    try:
        ...
        proto_bytes = major_login.SerializeToString()
        encrypted_payload = encrypt_with_aes(proto_bytes)
...
def _encrypt_major_login_proto(open_id: str, access_token: str, region: str, client_ip: str) -> Optional[bytes]:
    """Encrypts MajorLogin protobuf for async generation"""
    ...
    try:
        ...
        return encrypt_with_aes(serialized)
    except Exception as e:
        print(f"Error encrypting major login proto: {e}")
        return None

async def _perform_major_login_async_api(
    ...
    """Performs MajorLogin asynchronously"""
    cfg = ACTIVATION_CONFIGS.get(region.upper())
    if not cfg:
...
async def _major_register_and_login_async_api(
    session: aiohttp.ClientSession,
    uid: str,
    password: str,
    access_token: str,
    ...
    """Performs MajorRegister and subsequent Login"""
    try:
...
async def handle_index(request: web.Request) -> web.Response:
    ...
    return web.json_response({
        ...
        "example": "https://your-service.onrender.com/g?name=Vaibhav&region=IND&count=5&activate=true",
        "features": {
