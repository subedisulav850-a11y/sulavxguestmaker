...
"""
================================================================================
GARENA GUEST GENERATOR API - PROFESSIONAL EDITION
================================================================================
Version: 2.0.0
Python: 3.11+ (with 3.14 fallback)
Deployment: Render.com / Any Cloud
================================================================================
"""
...
# =============================================================================
# PYTHON 3.14 COMPATIBILITY FIX - CRITICAL FOR RENDER
# =============================================================================
print("🐍 Python Version:", sys.version)
print("📦 Loading dependencies...")

# -----------------------------------------------------------------------------
# PROTOBUF IMPORTS WITH FALLBACK FOR PYTHON 3.14
# -----------------------------------------------------------------------------
try:
    import MajoRLoGinrEq_pb2
    import MajoRLoGinrEs_pb2
    NEW_PROTO_AVAILABLE = True
    print("✅ Protobuf loaded successfully")
except ImportError as e:
    print(f"⚠️ Protobuf import error (ImportError): {e}")
    print("⚠️ Running in fallback mode - account generation will use backup methods")
    ...
    class DummyMajorLogin(DummyProto):
        pass
        
    class DummyMajorLoginRes(DummyProto):
        pass
    
    MajoRLoGinrEq_pb2 = type('MajoRLoGinrEq_pb2', (), {
        'MajorLogin': DummyMajorLogin
    })()
    
    MajoRLoGinrEs_pb2 = type('MajoRLoGinrEs_pb2', (), {
        'MajorLoginRes': DummyMajorLoginRes
    })()
    
except TypeError as e:
    print(f"⚠️ Protobuf type error (Python 3.14+ issue): {e}")
    print("⚠️ Using fallback protobuf implementation")
    NEW_PROTO_AVAILABLE = False
    ...
    print(f"⚠️ Unexpected protobuf error: {e}")
...
try:
    ...
    AES_AVAILABLE = True
    print("✅ Crypto (pycryptodome) loaded successfully")
except ImportError as e:
    print(f"⚠️ Crypto import error: {e}")
    print("⚠️ Using fallback crypto - encryption disabled")
    AES_AVAILABLE = False
...
# =============================================================================
REGION_IP_RANGES = {
    "IND": ["103.21.140.", "103.51.92.", "43.224.128.", "115.240.0.", "103.24.14.", "103.15.78."],
    "TW":  ["1.160.0.", "36.224.0.", "61.216.0.", "114.24.0.", "118.160.0.", "111.240.0."],
    "BD":  ["103.25.248.", "103.102.116.", "115.127.24.", "103.80.204.", "103.174.112."],
    "PK":  ["39.32.0.", "111.88.0.", "119.160.0.", "101.50.112.", "175.107.128."],
    "ID":  ["36.64.0.", "101.255.0.", "114.120.0.", "202.152.0.", "125.160.0."],
    "TH":  ["1.4.0.", "49.228.0.", "58.8.0.", "118.172.0.", "61.90.0."],
    "VN":  ["1.52.0.", "14.160.0.", "113.160.0.", "117.0.0.", "171.224.0."],
    "ME":  ["2.50.0.", "5.30.0.", "82.199.0.", "94.128.0.", "188.48.0."],
    "BR":  ["2.80.0.", "177.0.0.", "186.200.0.", "187.0.0.", "191.0.0."],
    "EU":  ["46.4.0.", "95.140.0.", "109.252.0.", "91.64.0.", "85.212.0."],
    "CIS": ["46.0.0.", "95.52.0.", "178.120.0.", "92.80.0.", "85.140.0."],
    "NA":  ["3.0.0.", "63.160.0.", "128.0.0.", "172.0.0.", "198.0.0."],
    "SAC": ["181.1.0.", "190.1.0.", "200.1.0.", "191.96.0.", "191.97.0."]
}
...
def generate_exponent_number():
    """Generates a number with superscript digits for unique names"""
    exponent_digits = {
        '0': '⁰', '1': '¹', '2': '²', '3': '³', '4': '⁴',
        '5': '⁵', '6': '⁶', '7': '⁷', '8': '⁸', '9': '⁹'
    }
...
def decode_jwt_token(jwt_token):
    """Decodes JWT token to extract account ID"""
    try:
...
def build_safe_major_login_payload(open_id, access_token, region, client_ip):
    ...
    try:
        ...
        return encrypt_with_aes(proto_bytes)
    except Exception as e:
        print(f"Error building major login payload: {e}")
        return None
...
def parse_safe_major_login_response(response_bytes):
    ...
    try:
        ...
        print(f"Error parsing major login response: {e}")
        return None
...
def build_get_login_data_payload(jwt_token, access_token, region, client_ip):
    try:
        ...
        print(f"Error building get login data payload: {e}")
        return None
...
def _encrypt_major_login_proto(open_id, access_token, region, client_ip):
    ...
    try:
        ...
        print(f"Error encrypting major login proto: {e}")
        return None
    ...
    try:
        async with session.post(
            ...
            if resp.status == 200:
                ...
                # Fallback: Try to extract JWT from text
        ...
        print(f"MajorLogin async error: {e}")
        pass
    return {"account_id": "N/A", "jwt_token": ""}
...
async def _major_register_and_login_async_api(session, uid, password, access_token, open_id, name, region, is_ghost, client_ip, proxy_url=None):
    try:
        ...
        if login_result is None or login_result.get("account_id") == "N/A":
            return None
    ...
    async with semaphore:
        for attempt in range(5):  # 5 fast retries
            try:
                ...
                if account_data and account_data.get('account_id') != "N/A":
                    ...
                    if auto_activate and not is_ghost:
                        try:
                            ...
                            print(f"Activation error for {uid}: {e}")
                ...
                print(f"Account generation attempt {attempt + 1} failed: {e}")
                continue
        return None
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            ...
            for account_data in completed:
                if isinstance(account_data, Exception):
                    print(f"Task failed with exception: {account_data}")
                    continue
                if account_data and account_data.get('account_id') != "N/A":
...
async def handle_index(request):
    return web.json_response({
        ...
        "example": "https://sulavxguestmaker.onrender.com/g?name=Vaibhav&region=IND&count=5&activate=true",
        "features": {
            "auto_activation": "Automatically activates generated accounts",
            "ip_rotation": "Regional IP rotation for each request",
            "proxy_support": "Optional proxy rotation via proxies.json",
            "concurrency": "Supports up to 20 concurrent requests"
        }
    })
...
async def handle_health(request):
    return web.json_response({
        ...
        "protobuf_available": NEW_PROTO_AVAILABLE,
        "crypto_available": AES_AVAILABLE
    })
...
def main():
    ...
    print("=" * 70)
    print("🚀 GARENA GUEST GENERATOR API - PROFESSIONAL EDITION")
    print("=" * 70)
    ...
    print(f"  • Generate 3 accounts for India: curl http://0.0.0.0:{port}/g?region=IND&count=3")
    print(f"  • Generate 5 accounts with auto activation: curl http://0.0.0.0:{port}/g?region=ID&count=5&activate=true")
    print(f"  • Generate 2 accounts without activation: curl http://0.0.0.0:{port}/g?region=TW&count=2&activate=false")
    print("=" * 70)
    print(f"✅ Service Status:")
    print(f"   • AES Available: {AES_AVAILABLE}")
    print(f"   • Protobuf Available: {NEW_PROTO_AVAILABLE}")
